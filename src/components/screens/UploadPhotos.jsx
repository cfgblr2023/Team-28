...
const UploadPhotos = () => {
  ...
  const {activeSessionId} = useTimerContext()
  const startCamera = () => {
  ...
  const uploadImage = (dataURL) => {
    ...
    navigator.geolocation.getCurrentPosition(
      (position) => {
        ...
        const url = "http://localhost:5000/api/users/createIssue/" + activeSessionId.toString()
        fetch(url, {
          method: "POST",
          body: JSON.stringify({
            photo: dataURL,
          ...
          .then((data) => {
            ...
            // Reset the camera
            setImageData(null);
            navigation(`/session/${activeSessionId}`)
          })
          .catch((error) => {
    ...
    <div className="flex h-full flex-col items-center">
      ...
      <div className="w-full absolute flex justify-center mb-5 items-center bottom-0 h-auto">
        {!imageData ? (
          ...
          <div className="w-full absolute flex flex-col justify-center mb-5 items-center bottom-0 h-auto">
            <input
              value={description}
              className="w-50 rounded-md h-50 bg-gray-100"
